...
Version avec connexion directe (sans variable d'environnement).
"""
...
def get_mysql_connection_direct():
    ...
    try:
        # Informations de connexion directes (à modifier selon vos besoins)
        host = "localhost"  # ou l'adresse de votre serveur MySQL
        port = 3306
        user = "cmtch_user"  # votre nom d'utilisateur MySQL
        password = "votre_mot_de_passe_mysql"  # votre mot de passe MySQL
        database = "cmtch_db"  # nom de votre base de données
        
        print(f"🔌 Connexion à MySQL: {host}:{port}/{database}")
        print("⚠️  Vérifiez que les informations de connexion sont correctes dans le script")
        ...
        print("   1. Modifiez les variables host, user, password, database dans ce script")
        print("   2. Ou configurez la variable d'environnement DATABASE_URL")
        return None
...
def main():
    ...
    print("=" * 70)
    print("⚠️  ATTENTION: Ce script nécessite la configuration des informations MySQL")
    print("   Modifiez les variables dans get_mysql_connection_direct() selon vos besoins")
    print()
